...
export function useRunningJobs(userId: string | null | undefined) {
  ...
  useEffect(() => {
    ...
    // Realtime already covers every INSERT/UPDATE on the two watched tables.
    // A 60s safety refresh is kept as a fallback for realtime disconnects; the
    // previous 20s poll ran app-wide per signed-in user and tripled backend
    // load without correctness gain.
