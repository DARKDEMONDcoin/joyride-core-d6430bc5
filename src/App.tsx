...
const AnythingPage = lazy(() => import("./pages/anything/AnythingPage"));
...
const LearnDashboardPage = lazy(() => import("./pages/learn/LearnDashboardPage"));
...
// Suspense backstop. This only shows on a cold load of a route (no previous
// page to keep painted) — never between in-app navigations, since routes are
// rendered from a deferred location. It always paints the themed background so
// the user never sees a bare black frame.
const LazyFallback = () => {
  const location = useLocation();
  const isChatRoute =
...
const App = () => {
  ...
  useEffect(() => {
    // Forced dark-only theme. Light theme has been removed per product decision.
    ...
    // RTL global direction (imported settings sourced from MDN + Tailwind RTL plugin guidance).
    ...
    // Keep theme forced to dark even if other code emits a change event.
